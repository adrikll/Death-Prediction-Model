...
class ChagasDataset(Dataset):
    ...
    def _extract_ecg_features(self, signal: np.ndarray) -> pd.Series:
        ...
        signals_proc, info = nk.ecg_process(segment, sampling_rate=self.sampling_rate)
    ...
    def __getitem__(self, idx):
        ...
        if not m:
            raise ValueError(f"Arquivo com nome inválido: {basename}")
...
def save_features(dataset, output_file):
    ...
    for idx in range(len(dataset)):
        try:
            ...
            class_1 = 1 if (obito == 1 & tempo < 5) else 0
